...
class Scenario:
    ...
    def from_config(cls, Env, Solver, config):
        # Create basic class: controller, counter, recorder
        #print("_____________rania__0________",Env,"\n\n",Solver,"\n\n",config,"------done--------\n\n",)
        ...
        # Create p_net (if data exist or create it ,and v_net simulator
    ...
    def ready(self):   # ICI ou le model learn, or eval mode or just load, i have to see the attribute load model.. where it is modified


        # Tout dépend de l'attribute  hasattr(self.solver, if load or learn or eval
        # load pretrained model
        if hasattr(self.solver, 'load_model') and self.config.pretrained_model_path not in ['None', '']:
           # print("_____________$_______load_model_______________________________")
        ...
        if hasattr(self.solver, 'learn') and self.config.num_train_epochs > 0:
           # print("______________$______learn_______________________________")
            print(f"\n{'-' * 20}  Pretrain  {'-' * 20}\n")
        ...
        if hasattr(self.solver, 'eval'):
            print("______________$______eval_______________________________")
            self.solver.eval()  ##########
...
class BasicScenario(Scenario):
    ...
    def run(self):
        print("_____________rania__1________")
        self.ready()
        ...
        for epoch_id in range(self.config.start_epoch, self.config.start_epoch + self.config.num_epochs):
            ...
            print("______scen env______",self.env)
            ...
            #print("________________pbar_____________",pbar)
            while True:#### attention boucle infinie , jusqu'a soltion gives done=true #1

                #instance == observation , by me
                # solver == agent 
                # solution == action (matching)
                solution = self.solver.solve(instance)
                print("________________instance____________",instance)

                print("________________solution_____________",solution)

                next_instance, _, done, info = self.env.step(solution) #1
                print("________________next instance____________",next_instance,done,info)
