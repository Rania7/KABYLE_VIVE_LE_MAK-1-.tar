...
class RLSolver(Solver):
    ...
    def __init__(self, controller, recorder, counter, **kwargs):
        ...
        self.buffer_ddpg=RolloutBufferDdpg()
        # save
    ...
    def select_action(self, observation, mask=None, sample=True):
        ...
        f_logitis=open("logitis.txt","w")
        f_logitis.write("action_logits\n"+str(action_logits)+"\n\n")
        f_logitis.write("action_logits\n"+str(action_logits.size())+"\n\n")
        

        # mask est appliqué pour réduire la proba de certaine
        # action à 0, les actions interdites 

        #il mets à 0 les proba entre [-1,1] -1 et 1 compris
        ...
        f_logitis.write("candicate_action_logits\n"+str(candicate_action_logits)+"\n\n")


        if self.mask_actions and self.maskable_policy:
            f_logitis.write("mask\n"+str(mask)+"\n\n")
        ...
        f_logitis.write("candicate_action_probs\n"+str(candicate_action_probs)+"\n\n")
        f_logitis.write("candicate_action_dist\n"+str(candicate_action_dist)+"\n\n")

        if sample:
            action = candicate_action_dist.sample()
            f_logitis.write("candicate_action_dist.sample()\n"+str(candicate_action_dist.sample())+"\n\n")

        else:
            action = candicate_action_logits.argmax(-1)
            f_logitis.write("candicate_action_logits.argmax(-1)\n"+str(candicate_action_logits.argmax(-1))+"\n\n")


        action_logprob = candicate_action_dist.log_prob(action)
        f_logitis.write("action_logprob\n"+str(action_logprob)+"\n\n")
        action = action.reshape(-1, )
        
        f_logitis.write("action\n"+str(action)+"\n\n")
        f_logitis.write("action.cpu().detach().numpy(), action_logprob.cpu().detach().numpy()\n"+str(action.cpu().detach().numpy())+"\t"+str(action_logprob.cpu().detach().numpy())+"\n\n")
...
class InstanceAgentDdpg(object):
    """Training and Inference Methods for Resource Allocation Agent"""
    def __init__(self) -> None: #
        pass
    ...
    def validate(self, env, checkpoint_path=None):  # vérifié normalment pas besoin de modifier
        print(f"\n{'-' * 20}  Validate  {'-' * 20}\n") if self.verbose >= 0 else None
        if checkpoint_path is not None: self.load_model(checkpoint_path)
    ...
    def get_baseline_solution_info(self, instance, use_baseline_solver=True):  # vérifié et normalment pas besoin de modifier
        """
        Get the baseline solution info for the instance, including:
            - result: whether the baseline solution is feasible
            - v_net_r2c_ratio: the revenue to cost ratio of the baseline solution
    ...
    def learn_with_instance(self, instance):#
        print("learn_with_instance du rl_solver not solver")
        # sub env for sub agent
        sub_buffer = RolloutBufferDdpg()
        v_net, p_net = instance['v_net'], instance['p_net']


        file_v_v_net= open("v_p_net.txt","a")
        file_v_v_net.write(str(v_net)+"----cbon----\n\n")
        file_v_v_net.write(str(p_net)+"----cbon----\n\n")
        file_v_v_net.close()
        ...
        while not sub_done:
            ...
            sub_buffer.add(sub_obs, action, sub_reward, sub_done, action_logprob, next_sub_obs,value=value)
            sub_buffer.action_masks.append(mask)
    ...
    def merge_instance_experience(self, instance, solution, sub_buffer,instance_,reward):
        ### -- use_negative_sample -- ##
        if self.use_negative_sample:
            baseline_solution_info = self.get_baseline_solution_info(instance, self.use_baseline_solver)
            if baseline_solution_info['result'] or solution['result']:
                #sub_buffer.compute_returns_and_advantages(last_value, gamma=self.gamma, gae_lambda=self.gae_lambda, method=self.compute_advantage_method)
                self.buffer_ddpg.merge(sub_buffer)
                self.time_step += 1
            else:
                pass
        elif solution['result']:  #  or True
            ...
            pass
        return self.buffer_ddpg


    # equivalent main in ddpg
    def learn(self, env, num_epochs=1, start_epoch=0, save_timestep=1000, config=None, **kwargs):
        # main env
        self.start_time = time.time()
        ...
        for epoch_id in range(start_epoch, start_epoch + num_epochs):
            ...
            #instance future == observation future
            instance_={}


            for i in range(env.num_v_nets): ## instance == virtual network
                ### --- instance-level --- ###
                print("^^^^^^^BUFFER^^^^^^^^^^")
                solution, sub_buffer = self.learn_with_instance(instance) #choose action
                ###################me 
                file_buffer= open("file_buffer.txt","w")
                
                file_buffer.write(str(sub_buffer.observations)+"\n")
                file_buffer.write("\n___1__\n")
                file_buffer.write(str(sub_buffer.actions)+"\n")
                file_buffer.write("\n____2_\n")
                file_buffer.write(str(sub_buffer.rewards)+"\n")
                file_buffer.write("\n____3_\n")

                file_buffer.write(str(sub_buffer.dones)+"\n")
                file_buffer.write("\n___4__\n")

                file_buffer.write(str(sub_buffer.next_observations)+"\n")
                ...
                instance_, reward, done, info = env.step(solution)
                self.merge_instance_experience(instance, solution, sub_buffer,instance_,reward)

                # update parameters
                if self.buffer_ddpg.size() >= self.target_steps:
                    loss = self.update()
                    # print(f'loss: {loss.item():+2.4f}, mean r2c: {np.mean(revenue2cost_list):+2.4f}')
...
class DdpgSolver(RLSolver,object): #A2CSolver

    def __init__(self, controller, recorder, counter, **kwargs):
        super(DdpgSolver, self).__init__(controller, recorder, counter, **kwargs)
        self.repeat_times = 1
        self.batch_size=30
    def update(self, ):  ## ici equivalent learn in ddpg
        ################################################

        ##################################################states, actions, rewards, states_, done = \
        if self.buffer_ddpg.curr_idx < self.batch_size:
            return

        states, actions, rewards, states_, done = \
                self.buffer_ddpg.sample_buffer(self.batch_size)
        ...
        print("lol 1")
        #pass tensor in forward
        target_actions = self.target_actor.forward(next_observations)#1
        critic_value_ = self.target_critic.forward(next_observations, target_actions)#2 
        
        critic_value = self.critic.forward(observations, actions) #true value
        ...
        #########stop here
        ############################### i save it just for entropy loss, entropy loss just to print it, 
        ############################### dont optùize using it '''  '''
        '''
        if len(self.buffer.action_masks) != 0 and self.mask_actions:
            masks = torch.IntTensor(np.concatenate(self.buffer.action_masks, axis=0)).to(self.device)
        else:
        ...
        #self.buffer_ddpg.clear()
        self.lr_scheduler.step() if self.lr_scheduler is not None else None
        self.update_time += 1
        return actor_loss #my choice
    ...
    def estimate_obs(self, observation, action):
        return self.policy.evaluate(observation, action).squeeze(-1).detach().cpu()
    ...
    def select_action(self, observation, mask=None, sample=True):
        ...
        f_logitis.close()
        # action = action.squeeze(-1).cpu()
        return action.cpu().detach().numpy(), action_logprob.cpu().detach().numpy(),action_logits
