...
class A3CGcnSeq2SeqSolver(InstanceAgent, A2CSolver):
    ...
    def learn_with_instance(self, instance):
        ...
        f.write("\action_logprob\n")
        f.write(str(action_logprob))
        f.write("\n\n"+str(len(action_logprob)))

        f.write("\naction[0]\n")
        f.write(str(action[0]))

        f.write("\n p_node_id next \n")
        f.write(str(action[0].item()))
        ...
        f.write("\n solution  \n")
        f.write(str(solution['node_slots']))
        f.close()
