...
class ActorCritic(nn.Module):
    ...
    def __init__(self, p_net_num_nodes, p_net_feature_dim, v_net_feature_dim, embedding_dim=64):
        ...
        file_logits=open("logit_file","a")
        file_logits.write("p_net_num_nodes "+str(p_net_num_nodes)+"\n\n")
        file_logits.close()
    ...
    def act(self, obs):
        ...
        file_logits=open("logit_file","a")
        file_logits.write("logits"+str(logits)+"\n\n")
        file_logits.close()
...
class Decoder(nn.Module):
    ...
    def forward(self, obs):
        ...
        f=open("obs_actor_a3C.txt","w")
        f.write(str(p_node_embeddings))
        f.write(str(p_node_embeddings.size())+"\n\n")
        f.write(str(logits))
        f.write(str(logits.size()))
        f.close()
        return logits, outputs, hidden_state
