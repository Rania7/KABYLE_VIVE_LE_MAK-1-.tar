...
class RolloutBufferDdpg:
    
    def __init__(self, max_size=50):
        self.curr_idx = 0
        self.max_size = max_size

        self.basic_items = ['observations', 'actions','next_observations', 'rewards', 'dones', 'logprobs', 'values']
    ...
    def add(self, obs, action, raward, done, logprob, next_obs, value=None):
        f=open("add_in_buffer.txt","a")
        f.write("lol"+str(self.curr_idx))
        f.close()
    ...
    def sample_buffer(self, batch_size): #me
        print("____________I am here____________",self.curr_idx, self.max_size)
        max_mem = min(self.curr_idx, self.max_size)
        ...
        states = self.observations[batch]
        actions = self.actions[batch]
        rewards = self.rewards[batch]
        states_ = self.next_observations[batch]
        terminates = self.dones[batch]
